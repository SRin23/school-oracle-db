...
--EQUI JOIN
--예제1
SELECT EMPNO, ENAME, EMP.DEPTNO E_DEPTNO, DEPT.DEPTNO D_DEPTNO, DNAME
FROM DEPT, EMP
...
--컬럼 별칭
SELECT ENAME NAME, SAL, SAL*12 AS "Annual Salary"
FROM EMP;
...
--연결 연산자
SELECT ENAME||' : '||EMPNO||' : '||JOB AS "사명 : 사번 : 업무"
FROM EMP;
...
SELECT EMPNO, ENAME, JOB, CONCAT(EMPNO, ENAME) E_NAME, CONCAT(ENAME, EMPNO) E_EMPNO, CONCAT(ENAME, JOB) E_JOB
FROM EMP
WHERE DEPTNO = 10;
...
SELECT INSTR(ENAME, 'L') E_NULL, INSTR(ENAME, 'L', 1, 1) E_11,
